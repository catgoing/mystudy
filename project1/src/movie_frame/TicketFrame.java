...
public class TicketFrame extends JFrame implements ItemListener, ActionListener{
	mainView mv;
		...
		int totalP = 0;
	      ...
	      if((adult_ticket == 0 && child_ticket == 0) || strmovie_time.equals(null))
	      {
	         JOptionPane.showMessageDialog( null, "정보를 모두 입력하세요", "error", JOptionPane.ERROR_MESSAGE );
	          ...
	          SeatFrame seat = new SeatFrame(movieName.getItem(select),adult_ticket + child_ticket,
	                                              totalP, ticketNum, strmovie_time);
	            dispose();
	       }
